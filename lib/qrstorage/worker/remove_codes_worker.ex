defmodule Qrstorage.Worker.RemoveCodesWorker do
  ...
  defp delete_recordings(deleted_codes) do
    ...
    recording_qr_codes_to_delete = qrcodes_by_filtered_codes(deleted_codes, :recording)

    if Enum.empty?(recording_qr_codes_to_delete) do
      ...
      StorageService.delete_recordings(recording_qr_codes_to_delete)
    end
  end

  defp delete_tts(deleted_codes) do
    # filter for recording codes, we need to delete the content from the object storage:
    tts_qr_codes_to_delete = qrcodes_by_filtered_codes(deleted_codes, :audio)

    if Enum.empty?(tts_qr_codes_to_delete) do
      Logger.info("No tts codes to delete from object storage.")
    else
      Logger.info("Deleting tts codes with id from object storage.")
      StorageService.delete_tts(tts_qr_codes_to_delete)
    end
  end

  defp qrcodes_by_filtered_codes(codes, filter) do
    codes
