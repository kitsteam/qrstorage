defmodule Qrstorage.Services.RecordingServiceTest do
  ...
  describe "extract_recording_from_params/1" do
    ...
    test "returns {:ok, audio_file, audio_file_type} when file is present", %{tmp_dir: tmp_dir} do
      ...
      params = %{
        "audio_file" => %Plug.Upload{path: file_path, content_type: "webm", filename: "recording.webm"}
      }

      assert {:ok, "binary audio content", "webm"} == RecordingService.extract_recording_from_params(params)
    end
    ...
    test "returns {:error, message} when file is not present", %{tmp_dir: tmp_dir} do
      ...
      params = %{
        "audio_file" => %Plug.Upload{path: file_path, content_type: "webm", filename: "not-written.webm"}
      }
